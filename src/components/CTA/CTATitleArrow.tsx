...
const CTATitleArrow = ({ data }: { data: CTATitleArrowProps }) => {
  const {
    title,
    description,
    buttonText,
  ...
  return (
    <section className="">
      <div
        className={`container flex flex-col items-center rounded-3xl justify-center py-32 ${bgColor}`}
      >
        <div className="flex">
        ...
        <header>
          <h1
            className={`mt-6 text-center text-4xl font-semibold tracking-tighter ${titleTextColor} lg:text-5xl`}
          >
            {title}
          </h1>
          <p
            className={`mx-auto mt-6 max-w-2xl text-center text-lg ${descriptionTextColor} md:text-2xl`}
          >
            {description}
          </p>
        </header>
        <div className="group">
          <div className="relative flex items-center justify-center">
            <Link href={buttonLink}>
              <Button
                className={`mt-6 h-12 rounded-2xl px-6 text-lg ${buttonBgColor} ${buttonTextColor}`}
              >
                {buttonText}
              </Button>
              <div className="absolute -right-25 -bottom-20 flex items-end md:-right-52">
                <Line />
                <p className={`font-caveat text-xl tracking-tight ${titleTextColor}`}>
                  {buttonSubtext}
                </p>
              </div>
            </Link>
